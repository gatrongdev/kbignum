...
- Tạo `enum`/`sealed class` mô tả các chế độ làm tròn trong common module và cung cấp mapping rõ ràng xuống hai nền tảng.
- Sắp xếp lại logic `divide`/`setScale` trên Android & iOS để dùng chung bảng luật, kèm test với số âm/dương.
...
- Di chuyển giá trị `compileSdk`, `minSdk`, `targetSdk` lên cấu hình cha (`gradle.properties` hoặc `libs.versions.toml`).
- Trong `shared/build.gradle.kts`, đọc các thuộc tính này để module tự đồng bộ khi project cha đổi SDK.
- Ghi chú hướng dẫn override trong README/CHANGELOG.
...
1. Thiết kế abstraction làm tròn mới và mapping chi tiết.
2. Thiết lập test chéo cho `divide`/`setScale` (bao gồm số âm).
3. Triển khai arithmetic chuỗi trên iOS và benchmarking tối thiểu.
4. Refactor build script đọc SDK từ cấu hình cha, cập nhật README/CHANGELOG.
