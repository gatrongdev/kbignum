...
class KBigInteger(
    ...
    companion object {
        ...
        fun fromString(value: String): KBigInteger {
            ...
            // Process first group
            val firstChunk = value.substring(cursor, cursor + firstGroupLen).toInt()
            result = fromInt(firstChunk)
            cursor += firstGroupLen
            
            while (cursor < len) {
                val chunk = value.substring(cursor, cursor + 9).toInt()
    ...
    fun toLong(): Long {
        ...
        var res = magnitude[0].toLong() and 0xFFFFFFFFL
        if (magnitude.size > 1) {
            res = res or ((magnitude[1].toLong() and 0xFFFFFFFFL) shl 32)
        }
        return if (signum < 0) -res else res
    }
    ...
    fun toPreciseNumber(): KBigDecimal {
        // Temporary Stub
        throw NotImplementedError("KBigDecimal dependency pending")
    }
    ...
    fun divide(other: KBigInteger): KBigInteger {
        ...
        return KBigInteger(resultSign, quotientMag)
    }
    ...
    fun mod(other: KBigInteger): KBigInteger {
        ...
        return KBigInteger(signum, remainderMag)
    }
