...
class KBigDecimalComplianceTest {
    ...
    private fun generateHugeRandomPair(bits: Int): Pair<BigDecimal, BigDecimal> {
        ...
        val aUnscaled = java.math.BigInteger(aBytes)
        val bUnscaled = java.math.BigInteger(bBytes)
    ...
    private fun mapRoundingMode(mode: java.math.RoundingMode): KBRoundingMode {
        return when(mode) {
    ...
    private fun KBigDecimal.toJavaBigDecimal(): BigDecimal {
        val unscaledStr = this.unscaledValue.toString()
        val javaUnscaled = java.math.BigInteger(unscaledStr)
