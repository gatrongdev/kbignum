...
class KBigIntegerBitwiseComplianceTest {
    ...
    fun testBitwiseOperationsAgainstJava_FuzzTest() {
        ...
        repeat(1000) { iter ->
            ...
            try {
                // AND
                assertEquals("AND failed (iter $iter) for $finalJavaA and $finalJavaB",
                    finalJavaA.and(finalJavaB).toString(),
                    kA.and(kB).toString()
                )
                
                // OR
                assertEquals("OR failed (iter $iter) for $finalJavaA and $finalJavaB",
                    finalJavaA.or(finalJavaB).toString(),
                    kA.or(kB).toString()
                )
                
                // XOR
                assertEquals("XOR failed (iter $iter) for $finalJavaA and $finalJavaB",
                    finalJavaA.xor(finalJavaB).toString(),
                    kA.xor(kB).toString()
                )
                
                // AND NOT
                assertEquals("AND NOT failed (iter $iter) for $finalJavaA and $finalJavaB",
                    finalJavaA.andNot(finalJavaB).toString(),
                    kA.andNot(kB).toString()
                )
                
                // NOT
                assertEquals("NOT failed (iter $iter) for $finalJavaA",
