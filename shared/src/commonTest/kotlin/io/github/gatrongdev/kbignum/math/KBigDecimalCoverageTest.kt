...
class KBigDecimalCoverageTest {
    ...
    fun testRoundingModes() {
        // Val = 2.5
        val val2_5 = KBigDecimal.fromString("2.5")
        
        // 2.5 -> Round Up -> 3
        assertEquals("3", val2_5.setScale(0, KBRoundingMode.Up).toString())
        
        // 2.5 -> Round Down -> 2
        assertEquals("2", val2_5.setScale(0, KBRoundingMode.Down).toString())
        
        // 2.5 -> Ceiling -> 3
        assertEquals("3", val2_5.setScale(0, KBRoundingMode.Ceiling).toString())
        
        // 2.5 -> Floor -> 2
        assertEquals("2", val2_5.setScale(0, KBRoundingMode.Floor).toString())
        
        // 2.5 -> HalfUp -> 3
        assertEquals("3", val2_5.setScale(0, KBRoundingMode.HalfUp).toString())
        
        // 2.5 -> HalfDown -> 2
        assertEquals("2", val2_5.setScale(0, KBRoundingMode.HalfDown).toString())
        
        // 2.5 -> HalfEven -> 2 (2 is even)
        assertEquals("2", val2_5.setScale(0, KBRoundingMode.HalfEven).toString())
        
        // 3.5 -> HalfEven -> 4 (4 is even)
        val val3_5 = KBigDecimal.fromString("3.5")
        assertEquals("4", val3_5.setScale(0, KBRoundingMode.HalfEven).toString())
        
        // Negative Cases
        // -2.5
        val neg2_5 = val2_5.negate()
        
        // -2.5 -> Up -> -3 (Away from zero)
        assertEquals("-3", neg2_5.setScale(0, KBRoundingMode.Up).toString())
        
        // -2.5 -> Down -> -2 (Towards zero)
        assertEquals("-2", neg2_5.setScale(0, KBRoundingMode.Down).toString())
        
        // -2.5 -> Ceiling -> -2 (Towards positive infinity)
        assertEquals("-2", neg2_5.setScale(0, KBRoundingMode.Ceiling).toString())
        
        // -2.5 -> Floor -> -3 (Towards negative infinity)
        assertEquals("-3", neg2_5.setScale(0, KBRoundingMode.Floor).toString())
    }
    ...
    fun testRoundingUnnecessary() {
        ...
        assertFailsWith<ArithmeticException> {
            val2_5.setScale(0, KBRoundingMode.Unnecessary)
        }
    }
